...
As usual, the raw data needed some cleaning before we could analyze it. We had to transform the data and convert it into [models in Metabase](/learn/metabase-basics/getting-started/models). And of course this travel data is affected by where people are traveling _from_: the global distribution of people, as well as economic factors - not everyone has the means to travel - will affect where people travel _to_. For example, given that about 40% of us live in Asia, we'd expect to find travel destinations in Asia seeing a lot of traffic.
