...
Sandboxing is available on [Pro and Enterprise plans](/pricing/) as a way to specify what data people can access based on who they are. Our article on [row permissions](/learn/metabase-basics/administration/permissions/data-sandboxing-row-permissions) covered how to create a [basic sandbox](/docs/latest/permissions/data-sandboxes#types-of-data-sandboxes) that restricts rows based on a person's user attribute. As an example, we created a user, Ms. Brown, and gave her access to rows in the `People` and `Orders` tables that matched her `user_id` attribute.
...
When we open up the `Products` table using the [data browser](/learn/metabase-basics/querying-and-dashboards/data-browser), we can confirm that Ms. Brown can only see a list of the products she's ordered, and only the three columns we included in our saved `Products from Orders` question: `Title`, `Category`, and `Price`.
...
- [Data sandboxing: row-level permissions](/learn/metabase-basics/administration/permissions/data-sandboxing-row-permissions)
- [Data sandboxing documentation](/docs/latest/permissions/data-sandboxes)
- [Permissions overview](/docs/latest/permissions/introduction)
- [Guide to data permissions](/learn/metabase-basics/administration/permissions/data-permissions)
...
- [Brand your Metabase instance](/learn/metabase-basics/embedding/brand)
