...
For teams that want to make programmatic changes to many Metabase items or settings at once, it's best to set up [one instance per environment](#one-instance-per-environment). You can also mimic dev and prod environments for [dashboard maintenance](/learn/grow-your-data-skills/analytics/editing-dashboards) by creating [one collection per environment](#one-collection-per-environment).
