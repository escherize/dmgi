...
Custom destinations work for both SQL and query builder questions, as custom destinations override the default click behavior. You can also set up [crossing-filtering](/learn/metabase-basics/querying-and-dashboards/dashboards/cross-filtering) on a dashboard, so that people can click on a chart to update a filter.
