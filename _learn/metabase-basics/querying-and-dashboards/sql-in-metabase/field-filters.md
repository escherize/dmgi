...
For Metabase questions written in SQL, we can use [basic variable types](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/sql-variables)---Text, Number, and Date---to create simple SQL filter widgets. To create "smarter" filter widgets that can display options specific to the data in the filtered columns, such as to create a dropdown menu of values, we can use a special variable type called a [Field Filter](/docs/latest/questions/native-editor/sql-parameters#the-field-filter-variable-type).
...
Note that Metabase will automatically use a search box if the number of distinct values in the column is greater than 300, even if you select the dropdown option. Learn more about [editing metadata](/docs/latest/data-modeling/metadata-editing) in our documentation.
...
Check out our guide to [basic SQL input variables - Text, Number, and Date](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/sql-variables).
