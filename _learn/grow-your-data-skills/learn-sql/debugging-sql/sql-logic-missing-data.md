...
5. If you want to _add_ rows to your query result to fill in data that is empty, zero, or `NULL`, go to [How to fill in data for missing report dates](#how-to-fill-in-data-for-missing-report-dates).
...
For example, when you write a `LEFT JOIN`, the table that comes _before_ the `LEFT JOIN` clause in your query is “on the left". The rows from the table “on the right” (the table _after_ the `LEFT JOIN` clause) will be filtered out if they don’t meet your join condition(s) in the `ON` clause.
...
[common-reasons-for-sql-logic-errors]: ./sql-logic#common-reasons-for-unexpected-query-results
...
[etl-learn]: /learn/grow-your-data-skills/data-landscape/etl-landscape
