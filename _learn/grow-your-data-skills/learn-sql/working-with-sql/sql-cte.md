...
As you can see in the figure above, you can also save the CTE's subquery as a [snippet](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/snippets), but you'd be better off saving a subquery as a question. The rule of thumb to decide between a snippet and a saved question is that if a block of code can return results on its own, you may want to consider saving it as a question (see [Snippets vs saved questions vs views](/learn/metabase-basics/querying-and-dashboards/sql-in-metabase/organizing-sql)).
