...
You have data in one table that you want to combine with data from another table. To do that, you'll need to use a `JOIN` to tell your database how the rows from one table relate to the rows in the other table. (Note that joins only work with relational databases, and you can only [join tables that live in the same database](/learn/metabase-basics/querying-and-dashboards/questions/cross-db-joins).)
