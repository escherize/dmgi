...
If you're ready to get going, you can start with our [upgrade guide](/docs/latest/installation-and-operation/upgrading-metabase), or read on to learn what's new.
...
We'll discuss snippets in more detail in an upcoming blog post, so stay tuned. In the meantime, you can [check out the documentation on snippets](/docs/latest/questions/native-editor/snippets).
...
Instead of changing the language in the UI for each user based on their browser's language settings, Metabase now lets admins pick the language for the instance on the [Localization tab](/docs/latest/configuring-metabase/localization) of the Settings page in the Admin Panel. Users can also change their personal language preferences from their Account Settings page.
...
We've moved things around a bit in the [Data Model section](/docs/latest/data-modeling/metadata-editing): there are now tabs at the top to see all of the Segments and Metrics you've defined across your whole instance, so you no longer need to drill down through your tables to find a Segment or Metric.
