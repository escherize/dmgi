...
I have searched the Internet for a simple (open source) analytics platform that can be integrated with many data sources, including [MongoDB](/docs/latest/databases/connections/mongodb) in particular. Also important to me was the interactivity and [embedded analytics](/docs/latest/embedding/start) capabilities of the tools l looked into. I found Metabase to tick all the boxes and their pricing and open source offering made this even more attractive.

Having played around with Docker before, I decided to [test Metabase](/docs/latest/installation-and-operation/running-metabase-on-docker) and look around using a local version. I managed to get this running in a matter of minutes. After asking a couple of questions in Metabase, it quickly became clear that I needed a production version. To continue exploring without losing any of the work I had 2 options as recommended in the [install documentation](/docs/latest/installation-and-operation/installing-metabase): saving the data on mapped file storage or using a [PostgreSQL database](/data_sources/postgresql). I opted for the database option as it seemed simple enough.
...
**AWS Lightsail**: A way to build applications and websites fast with low-cost, pre-configured cloud resources. We will use this to host a Docker container with Metabase and also as a host for the PostgreSQL database.
...
**PostgresSQL**: A powerful, open-source object-relational database that we will use to store our Metabase (meta) data and configurations. This will allow us to update, turn off and rebuild the Docker container without losing our data.
...
4. Choose your instance plan.
    ...
    - To get started, I would recommend selecting the `2gb Ram, 1vCPU, 60gb SSD` instance. (At the time of this article’s publication, Amazon is offering three months free on this instance size)
...
- To install this, follow the instructions on How To Install PostgreSQL on Ubuntu 20.04 [Quick Start].
...
- Set up the PostgreSQL user account and password for Metabase to use for the connection. This will be required in the next step. Below are the commands to create a new user and password in PostgreSQL.
`sudo -u postgres psql -c "CREATE USER <username> WITH PASSWORD '<password>'"`

- This user account will also need to be in the Superuser group so that it has access to creating a new database. Run the command below to add the user to the Superuser group.
